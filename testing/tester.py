...
of ${VAR} with the cur definition of VAR (see the D command below).
...
def doCopy(to1, src, dir):
    try:
        doDelete(to1, dir)
        copyfile(join(src_dir, src), join(dir, to1))
    except OSError:
        raise ValueError("file {} could not be copied to {}".format(src, to1))
